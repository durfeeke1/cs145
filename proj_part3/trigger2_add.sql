...
--Verify constraint
SELECT * FROM ItemTime WHERE EXISTS (
   SELECT * FROM Bids WHERE Bids.ItemID = ItemTime.ItemID and (datetime(Bids.ItemTime) < datetime(ItemTime.Started) or datetime(Bids.ItemTime)>datetime(ItemTime.Ends)));
